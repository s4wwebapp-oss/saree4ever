-- ============================================
-- COMPLETE MIGRATION: Landing Page Video Table
-- Run this entire file in Supabase SQL Editor
-- ============================================

-- Create landing_page_videos table for managing homepage autoplay video
CREATE TABLE IF NOT EXISTS landing_page_videos (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  title TEXT,
  video_url TEXT,
  video_file_path TEXT,
  thumbnail_url TEXT,
  is_active BOOLEAN DEFAULT true,
  autoplay BOOLEAN DEFAULT true,
  muted BOOLEAN DEFAULT true,
  loop BOOLEAN DEFAULT true,
  display_order INTEGER DEFAULT 0,
  video_orientation TEXT DEFAULT 'horizontal',
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW()
);

-- Create index for active video lookup
CREATE INDEX IF NOT EXISTS idx_landing_page_videos_active 
ON landing_page_videos(is_active) WHERE is_active = true;

-- Create index for ordering
CREATE INDEX IF NOT EXISTS idx_landing_page_videos_display_order 
ON landing_page_videos(display_order) WHERE is_active = true;

-- Create function to update updated_at timestamp
CREATE OR REPLACE FUNCTION update_landing_page_videos_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Create trigger to auto-update updated_at
CREATE TRIGGER update_landing_page_videos_updated_at
  BEFORE UPDATE ON landing_page_videos
  FOR EACH ROW
  EXECUTE FUNCTION update_landing_page_videos_updated_at();

-- Insert default record (inactive)
INSERT INTO landing_page_videos (video_url, is_active, autoplay, muted, loop, display_order, video_orientation) VALUES
(
  NULL,
  false,
  true,
  true,
  true,
  0,
  'horizontal'
)
ON CONFLICT DO NOTHING;

-- Enable Row Level Security
ALTER TABLE landing_page_videos ENABLE ROW LEVEL SECURITY;

-- Policy: Anyone can read active videos
CREATE POLICY "Anyone can read active landing page video"
  ON landing_page_videos FOR SELECT
  USING (is_active = true);

-- Policy: Authenticated admins can manage videos
CREATE POLICY "Admins can manage landing page video"
  ON landing_page_videos FOR ALL
  USING (auth.role() = 'authenticated');





