...
export const api = {
  ...
  // Coupons
  coupons: {
    validate: (data: { code: string; cartTotal: number }) => post('/coupons/validate', data),
    getAll: (params?: { limit?: number; offset?: number }) => {
      const query = new URLSearchParams();
      if (params?.limit) query.append('limit', String(params.limit));
      if (params?.offset) query.append('offset', String(params.offset));
      const qs = query.toString();
      return get(`/coupons${qs ? `?${qs}` : ''}`);
    },
    create: (data: unknown) => post('/coupons', data),
    update: (id: string, data: unknown) => patch(`/coupons/${id}`, data),
  ...
  filterOptions: {
    // Public endpoints
    getActive: () => get('/filter-options/active').catch(() => ({ options: {} })),
    getTypes: () => get('/filter-options/types').catch(() => ({ types: [] })),
    getByType: (type: string) => get(`/filter-options/type/${type}`).catch(() => ({ options: [] })),
    // Admin endpoints
    getAll: () => get('/filter-options'),
    getById: (id: string) => get(`/filter-options/${id}`),
    create: (data: unknown) => post('/filter-options', data),
    update: (id: string, data: unknown) => put(`/filter-options/${id}`, data),
    delete: (id: string) => del(`/filter-options/${id}`),
    updateOrder: (updates: Array<{ id: string; display_order: number }>) =>
      put('/filter-options/order', { updates }),
  },
};
