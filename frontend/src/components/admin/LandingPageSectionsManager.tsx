...
// Section metadata for display when DB doesn't have section_name/description
const SECTION_METADATA: Record<string, { name: string; description: string }> = {
  hero_carousel: { name: 'Hero Carousel', description: 'Main hero banner carousel at the top of the page' },
  quick_categories: { name: 'Quick Categories', description: 'Quick access category icons (Blouses, Jewels, New Arrivals, Hot Deals)' },
  landing_videos: { name: 'Landing Videos', description: 'Video section showcasing products or brand story' },
  shop_by_category: { name: 'Shop by Category', description: 'Full category grid with images and links' },
  featured_products: { name: 'Featured Products', description: 'Highlighted products marked as featured' },
  collections: { name: 'Collections', description: 'Curated collections like Festive, Bridal, etc.' },
  reels: { name: 'Reels / Short Videos', description: 'Instagram Reels or YouTube Shorts style content' },
  stories: { name: 'Stories / Blog', description: 'Featured blog posts and brand stories' },
  testimonials: { name: 'Testimonials', description: 'Customer testimonials and reviews' },
  reviews: { name: 'Customer Reviews', description: 'Product reviews section' },
  about_preview: { name: 'About Preview', description: 'Brief about us section with link to full page' },
  why_choose_us: { name: 'Why Choose Us', description: 'Key selling points and benefits' },
};
...
export default function LandingPageSectionsManager() {
  ...
  const getSectionName = (section: LandingPageSection) => {
    return section.section_name || SECTION_METADATA[section.section_key]?.name || section.section_key;
  };

  // Helper to get description for a section
  const getSectionDescription = (section: LandingPageSection) => {
    return section.description || SECTION_METADATA[section.section_key]?.description || 'No description';
  };
