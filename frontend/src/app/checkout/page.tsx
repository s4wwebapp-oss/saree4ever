...
export default function CheckoutPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const calculatedSubtotal = subtotal ?? total;
      const tax_amount = calculatedSubtotal * 0.18; // 18% GST (calculated on full amount)
  ...
  const subtotalAmount = subtotal ?? total;
  const couponDiscount = discountTotal;
  const isCouponApplied = couponDiscount > 0 && !!appliedCoupon;

  // Calculate discount amount if eligible (for display - backend will verify and apply)
  const newUserDiscountAmount = !isCouponApplied && discountInfo?.eligible
    ? (subtotalAmount * discountInfo.discount_percentage) / 100
    : 0;
  const effectiveDiscount = isCouponApplied ? couponDiscount : newUserDiscountAmount;

  const tax = subtotalAmount * 0.18; // Tax calculated on full amount (backend does the same)
  const shipping = 0;
  const finalTotal = Math.max(0, subtotalAmount - effectiveDiscount) + tax + shipping;
          ...
          <div className="lg:col-span-1">
            <div className="border border-black p-6 sticky top-24">
              ...
              <div className="space-y-2 mb-4 text-sm">
                ...
                {!isCouponApplied && effectiveDiscount > 0 && (
                ...
                {isCouponApplied && (
                  <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded text-xs text-green-800">
                    <p>✨ Coupon {appliedCoupon?.code} is active on this order.</p>
                  </div>
                )}
              </div>
