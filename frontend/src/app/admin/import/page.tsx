            ...
            <div className="mt-6 text-sm text-gray-500 bg-gray-50 p-4 rounded">
              ...
              {activeTab === 'products' && (
                <div className="space-y-2">
                  <code className="block whitespace-pre-wrap text-xs font-semibold text-gray-700">
                    Required: name, base_price
                  </code>
                  <code className="block whitespace-pre-wrap text-xs">
                    Optional: sku, description, long_description, collection_id, category_id, type_id,
                    compare_at_price, primary_image_url, image_urls, tags, is_featured, meta_title, meta_description
                  </code>
                  <p className="text-xs text-amber-600 mt-2">
                    💡 SKU is optional - leave blank for auto-generation! Download sample CSV from Admin → Help.
                  </p>
                </div>
              )}
              {activeTab === 'variants' && (
                <div className="space-y-2">
                  <code className="block whitespace-pre-wrap text-xs font-semibold text-gray-700">
                    Required: product_sku, name
                  </code>
                  <code className="block whitespace-pre-wrap text-xs">
                    Optional: sku, price, color, stock_quantity
                  </code>
                  <p className="text-xs text-amber-600 mt-2">
                    💡 Import products first, then variants. SKU auto-generates if left blank.
                  </p>
                </div>
              )}
              {activeTab === 'stock' && (
              ...
              {result.errors.length > 0 && (
                <div className="mb-4">
                  ...
                  {result.error_report_url && (
                    <a
                      href={`${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '')}${result.error_report_url}`}
                      target="_blank"
