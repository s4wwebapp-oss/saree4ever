...
export default function NewReelVideoPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const type = searchParams.get('type') || 'reel';
  ...
  const [reelVideo, setReelVideo] = useState<Partial<ReelVideo>>({
    ...
    meta_keywords: [],
    is_featured: false,
    instagram_reel_url: type === 'reel' ? '' : null,
    youtube_short_url: type === 'video' ? '' : null,
    content_type: type === 'reel' ? 'reel' : 'video',
  });
  const [tagsInput, setTagsInput] = useState<string>('');

  useEffect(() => {
    if (type === 'reel') {
      setReelVideo(prev => ({ ...prev, content_type: 'reel', instagram_reel_url: prev.instagram_reel_url || '', youtube_short_url: null }));
    } else {
      setReelVideo(prev => ({ ...prev, content_type: 'video', youtube_short_url: prev.youtube_short_url || '', instagram_reel_url: null }));
    }
  }, [type]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const reelVideoData = {
        ...
        content_type: type === 'reel' ? 'reel' : 'video',
        // Clear the opposite URL type
        instagram_reel_url: type === 'reel' ? reelVideo.instagram_reel_url : null,
        youtube_short_url: type === 'video' ? reelVideo.youtube_short_url : null,
      };

      await api.blog.create(reelVideoData);
    ...
    <div className="p-8">
      <div className="mb-6">
        <h1 className="heading-serif-md mb-2">
          Create New {type === 'reel' ? 'Instagram Reel' : 'YouTube Video'}
        </h1>
        <p className="text-gray-600">Add a new {type === 'reel' ? 'Instagram Reel' : 'YouTube Short/Video'} to your stories</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder={`Enter title for ${type === 'reel' ? 'Instagram Reel' : 'YouTube Video'}`}
              />
            </div>
            ...
            {type === 'reel' ? (
              <>
                ...
                <input
                  ...
                  onChange={(e) => setReelVideo({ ...reelVideo, instagram_reel_url: e.target.value })}
                  required
                  className="w-full px-3 py-2 border border-gray-300 focus:outline-none focus:ring-1 focus:ring-black"
                  placeholder="https://www.instagram.com/reel/ABC123/ or https://www.instagram.com/p/ABC123/"
                />
                <p className="text-xs text-gray-500 mt-1">
                  The video will be embedded and playable directly on your site
                </p>
              </>
            ) : (
              <>
                ...
                <input
                  ...
                  onChange={(e) => setReelVideo({ ...reelVideo, youtube_short_url: e.target.value })}
                  required
                  className="w-full px-3 py-2 border border-gray-300 focus:outline-none focus:ring-1 focus:ring-black"
        ...
        <div className="flex items-center justify-end space-x-4">
          ...
          >
            {saving ? 'Creating...' : `Create ${type === 'reel' ? 'Reel' : 'Video'}`}
          </button>
        </div>
      </form>
    </div>
